...
# EC2 / RDS example
# DATABASE_URL=postgresql+psycopg://db_username:db_password@your-rds-endpoint:5432/db_name?sslmode=require
...
※ インフラ構成（RDS等）の詳細は `infra/README.md` を参照してください。  
...
DBスキーマはAlembicで管理します。  
アプリケーション起動時の自動テーブル作成には依存せず、DB作成・変更時はmigrationを適用します。  
...
ホスト環境で実行する場合は、`DATABASE_URL` が接続可能なDBを指している状態で以下を実行します。  
...
ORMモデルを変更した後、以下のコマンドで差分からmigrationファイルを作成します。  
